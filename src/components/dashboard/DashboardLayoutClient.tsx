...
export default function DashboardLayoutClient({ pageSpecificContent }: { pageSpecificContent: React.ReactNode }) {
  useAuthRedirect(); 
    ...
    <SidebarProvider defaultOpen>
      <Sidebar>
        ...
        <SidebarFooter className="p-4 border-t border-sidebar-border">
            <Button variant="ghost" className="w-full justify-start text-sidebar-foreground hover:bg-sidebar-accent hover:text-sidebar-accent-foreground" disabled={!isMounted}>
      ...
      <SidebarInset>
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between gap-4 border-b bg-background/80 px-4 shadow-sm backdrop-blur-md sm:px-6">
          ...
          {!isMounted ? <Skeleton className="h-10 w-10 rounded-full" /> : <UserNav />}
        </header>
        <main className="flex-1 p-4 sm:p-6">
          {!isMounted ? (
            <div className="space-y-4">
              <Skeleton className="h-12 w-1/2 mb-6" /> {/* Page title skeleton */}
