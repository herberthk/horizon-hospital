...
const CallModal: React.FC<CallModalProps> = ({ isOpen, onClose }) => {
  ...
  const { callStatus,assistantIsSpeaking,audioLevel,messages,start,stop,callId } = useVapi();   const [lastMessage, setLastMessage] = useState<string>("");
      ...
      <DialogContent className="sm:max-w-[425px] bg-card text-card-foreground rounded-lg shadow-xl">
        <DialogHeader className="pt-6">
          <DialogTitle className="text-center text-lg italic font-semibold text-foreground">
            {callStatus === "LOADING" ? "Calling ai assistant..." : callStatus === "ERROR" ? "Something went wrong" : assistantIsSpeaking ? "AI assistant is speaking..." : "You're speaking with AI assistant"}
          </DialogTitle>
          <DialogDescription className="text-center text-muted-foreground italic px-4">
            {callStatus === "LOADING"
              ? "Getting things ready, please wait a moment." : callStatus === "ERROR"
