 ...
 * @returns Promise containing call data or undefined
 */
const pollForCallResults = async (id: string, maxAttempts = 10): Promise<Call | undefined> => {
    const API_KEY = process.env.VAPI_PRIVATE_KEY;
    let attempts = 0;
...
export const getAllCalls = async () => {
    try {
        const calls = await db.collection("calls").orderBy('timestamp', 'desc').get();
        return calls.docs.map(call => ({ ...call.data(),id:call.data().id } as Call) );
    } catch (error) {
